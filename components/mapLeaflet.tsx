...
function LocateUser({
  ...
  useEffect(() => {
    ...
    const handleLocationError = (e: L.ErrorEvent) => {
      console.log("위치 오류:", e);
      alert("위치 접근이 거부되었습니다. 마커를 드래그하여 위치를 선택하세요.");
      setLocated(true);
      map.off("locationfound", handleLocationFound);
      map.off("locationerror", handleLocationError);
    };
...
function ClickableMarker({
  ...
  const [markerPosition, setMarkerPosition] = useState<L.LatLngExpression>(position);
  ...
  return (
    <div className="w-full h-[380px] rounded-md overflow-hidden border">
      ...
      >
        ...
        <ClickableMarker position={markerPosition} onClick={handleClick} />
      </MapContainer>
