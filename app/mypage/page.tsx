    ...
    useEffect(() => {
        const fetchCounts = async () => {
            ...
            try {
                ...
                console.log('Fetching chat count for user ID:', user.user_id);
                const chatResponse = await fetch(
                    `/api/chat/users/${user.user_id}/count`,
                );
                const chatResult = await chatResponse.json();
                console.log('chatResult.data:', chatResult.data);
                if (chatResponse.ok) {
                    setChatCount(chatResult.data.count || 0);
                }
            } catch (error) {
            ...
            <div className="flex justify-center">
                <div className="flow-root border border-gray-200 rounded-xl p-10 m-10 w-90 h-auto ">
                    ...
                    <div className="mt-1 block w-full rounded-md border border-gray-200 bg-white px-3 py-2 text-gray-500 focus:border-amber-500 focus:ring-amber-500">
                        {userAddress == null
                            ? '동네 설정이 필요합니다.'
                            : `${userAddress.main} ${userAddress.dong}`}
                    </div>
                </div>
                <div className="flow-root border border-gray-200 rounded-xl p-6 m-10 w-150 h-auto">
                    <p className="text-lg font-bold text-center pb-5">
                        활동 대시보드
                    </p>
